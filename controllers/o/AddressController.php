...
class AddressController extends Controller
{
	...
	public function actionIndex() 
	{
		...
		$dataProvider = new CActiveDataProvider('DbsStudentAddress', array(
			'criteria'=>$criteria,
			'pagination'=>array(
				'pageSize'=>10,
			),
	...
	{
		$model=new DbsStudentAddress;

		// Uncomment the following line if AJAX validation is needed
		$this->performAjaxValidation($model);

		if(isset($_POST['DbsStudentAddress'])) {
			$model->attributes=$_POST['DbsStudentAddress'];
				...
				if(Yii::app()->getRequest()->getParam('enablesave') == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'id' => 'partial-dbs-student-address',
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'DbsStudentAddress success created.').'</strong></div>',
						));
					} else {
				...
				if(Yii::app()->getRequest()->getParam('enablesave') == 1) {
					if($model->save()) {
						echo CJSON::encode(array(
							...
							'get' => Yii::app()->controller->createUrl('manage'),
							'id' => 'partial-dbs-student-address',
							'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'DbsStudentAddress success updated.').'</strong></div>',
						));
					} else {
						print_r($model->getErrors());
					}
	...
	public function actionRunAction() {
		...
		if(count($id) > 0) {
			...
			if($actions == 'publish') {
				DbsStudentAddress::model()->updateAll(array(
					'publish' => 1,
				),$criteria);
				...
				DbsStudentAddress::model()->deleteAll($criteria);
			}
		}
	...
	public function actionDelete($id) 
	{
		...
		if(Yii::app()->request->isPostRequest) {
			...
			if(isset($id)) {
				if($model->delete()) {
					echo CJSON::encode(array(
						...
						'get' => Yii::app()->controller->createUrl('manage'),
						'id' => 'partial-dbs-student-address',
						'msg' => '<div class="errorSummary success"><strong>'.Yii::t('phrase', 'DbsStudentAddress success deleted.').'</strong></div>',
					));
				}
			}
			...
			$this->pageTitle = Yii::t('phrase', 'DbsStudentAddress Delete.');
			$this->pageDescription = '';
			$this->pageMeta = '';
	...
	{
		$model = DbsStudentAddress::model()->findByPk($id);
		if($model===null)
			throw new CHttpException(404, Yii::t('phrase', 'The requested page does not exist.'));
		return $model;
	}
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='dbs-student-address-form') {
